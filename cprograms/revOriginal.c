...
void reverseWord2 (char* buffer, int size);
void reverseLetter (char* buffer, int size);
void fatalError ();
...
int charOptionL = FALSE; /* Set to true if -l option is used */
int charOption2 = FALSE; /* second reverseWord method for testing */
...
/* Read a line and display it */
  ...
  if (charOptionL) reverseLetter (buffer, charsRead); //*******
  if (charOption2) reverseWord2 (buffer, charsRead); //method 2
...
void reverseLine (char* buffer, int size) {
/* Reverse all the characters in the buffer - comand line option cn*/
...
reverseWord2 (char* buffer, int length) {
/* Command line option 2, reverses the words */
...
void reverseLetter (char* buffer, int size) 
/* this reverses each letter without reversing the rest of the string */
{ 
    char* start = buffer; 
